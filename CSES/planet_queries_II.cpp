...
int query(int node1, int node2){
	...
	else if(nd[node1].conn_comp != nd[node2].conn_comp){
		return -1;
	}

	else{
		if(nd[node1].is_cycle && nd[node2].is_cycle){
			if(nd[node1].cycle_ind < nd[node2].cycle_ind){
				return (nd[node2].cycle_ind - nd[node1].cycle_ind);
			}

			else{
				return (nd[node2].cycle_size - nd[node1].cycle_ind + nd[node2].cycle_ind);
			}
		}

		else if(nd[node1].is_tree && nd[node2].is_cycle){
			return (query(nd[node1].parent,node2) + nd[node1].cycle_dist);
		}

		else if(nd[node1].is_tree && nd[node2].is_tree && nd[node1].parent == nd[node2].parent){
			int diff = nd[node1].cycle_dist - nd[node2].cycle_dist;

			if(diff <= 0){
				return -1;
			}

			if(jump(node1,diff) == node2){
				return diff;
			}
		}
...
signed main(){
	setIO(); int n,q; cin >> n >> q; graph.resize(n+1); visited.assign(n+1,0);
	forrange(i,1,n+1){
		int b; cin >> b; graph[i].pb(b); parent[i][0] = b;
	}
